...
export const eventsService = {
  ...
  async getCalendarEvents(startDate: string, endDate: string): Promise<Event[]> {
  ...
  async registerForEvent(eventId: string): Promise<{ qrCode: string }> {
    const response = await api.post<ApiResponse<{ qrCode: string }>>(`/events/${eventId}/register`);
    return response.data.data;
  },
  ...
  async getEventParticipants(eventId: string): Promise<PaginatedResponse<{ userId: string; name: string; email: string; status: string }>> {
  ...
  async approveEvent(eventId: string): Promise<Event> {
    const response = await api.post<ApiResponse<Event>>(`/events/${eventId}/approve`);
    return response.data.data;
  },

  // Reject event (admin only)
  async rejectEvent(eventId: string, reason?: string): Promise<Event> {
    const response = await api.post<ApiResponse<Event>>(`/events/${eventId}/reject`, { reason });
    return response.data.data;
  },
};
